...
│   ├── sizing.md
│   ├── yagni.md
│   ├── agents/         # Long-form docs for 8 agents that crossed the coverage rule, plus README
│   ├── skills/         # Long-form docs for all 15 skills, plus README
│   ├── guidance/       # Contributor-facing authoring guidance
│   └── templates/      # Templates and the coverage rule for long-form docs
└── images/             # Banner and graphics for README
```

The plugin is shipped from `plugin/`; documentation lives in `docs/`. Long-form docs in `docs/skills/{name}.md` and `docs/agents/{name}.md` are the canonical operator-facing source for any skill or agent that has one. The underlying definition (`plugin/skills/{name}/SKILL.md` or `plugin/agents/{name}.md`) is the implementation.
...
- **[docs/agents/project-manager.md](./docs/agents/project-manager.md)** — Facilitative coordinator of multi-specialist discussions; enforces evidence-based claims and synthesizes final plans.
- **[docs/agents/junior-developer.md](./docs/agents/junior-developer.md)** — Generalist stress-tester (three to five years of experience) that surfaces hidden assumptions, muddied scope, and uncited claims.
- **[docs/agents/software-architect.md](./docs/agents/software-architect.md)** — Adversarial intra-codebase architect; synthesizes structural/behavioral/concurrency/risk findings into SOLID-grounded recommendations.
- **[docs/agents/system-architect.md](./docs/agents/system-architect.md)** — Adversarial cross-service architect; operates where the unit of design is a service or bounded context.
- **[docs/agents/data-engineer.md](./docs/agents/data-engineer.md)** — Adversarial data engineer; audits schemas, migrations, queries, and pipelines against established data-engineering principles.
- **[docs/agents/devops-engineer.md](./docs/agents/devops-engineer.md)** — Adversarial DevOps / SRE; audits against DORA, Twelve-Factor, Four Golden Signals, and named production failure modes.
- **[docs/agents/information-architect.md](./docs/agents/information-architect.md)** — Adversarial documentation IA; audits findability, orientation, and comprehension against established IA frameworks.
- **[docs/agents/user-experience-designer.md](./docs/agents/user-experience-designer.md)** — Adversarial UX/IxD; audits against Nielsen heuristics, WCAG 2.2, universal design, and dark-pattern detection.

The other 13 agents — `adversarial-security-analyst`, `adversarial-validator`, `behavioral-analyst`, `codebase-explorer`, `concurrency-analyst`, `content-auditor`, `edge-case-explorer`, `evidence-based-investigator`, `gap-analyzer`, `project-scanner`, `risk-analyst`, `structural-analyst`, `test-engineer` — don't have long-form docs by design (see [docs/templates/coverage-rule.md](./docs/templates/coverage-rule.md)). The Agents Index links to their definitions in `plugin/agents/` directly.

### Templates (`docs/templates/`)
- **[docs/templates/skill-long-form-template.md](./docs/templates/skill-long-form-template.md)** — Template for a new skill's long-form doc. Use when adding a skill that qualifies for one.
...
- **[docs/templates/coverage-rule.md](./docs/templates/coverage-rule.md)** — Decision rule: when a skill or agent qualifies for a long-form doc and when the index entry is enough.
...
- **One canonical source per concept.** The long-form doc in `docs/skills/` or `docs/agents/` is canonical for that skill or agent. Index entries carry one-sentence scent + a link. The README never duplicates long-form content.
...
- **YAGNI applies to docs too.** Don't add speculative sections, for-future-flexibility warnings, or examples for behavior the skill doesn't yet have. The same evidence rule that gates plan steps gates docs.
- **Counts to verify when editing indexes.** 21 agents in `plugin/agents/`; 15 skills in `plugin/skills/`; 8 long-form agent docs in `docs/agents/`; 15 long-form skill docs in `docs/skills/`.
