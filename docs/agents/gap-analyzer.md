...
- **Pair with a validator swarm.** `/gap-analysis` optionally runs `adversarial-validator` and `evidence-based-investigator` against each gap. Opt in when stakeholders need confidence signals.
